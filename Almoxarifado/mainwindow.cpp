...
void MainWindow::dados_recebidos()
{
    ...
    if(dados.contains("LUZ"))
     ui->ln_luz->setText(dados["LUZ"].toString());
    if(dados.contains("TEMPERATURA"))
     ui->ln_temperatura->setText(dados["TEMPERATURA"].toString());
    if(dados.contains("PRESENCA"))
     ui->ln_presenca->setText(dados["PRESENCA"].toString());
}
